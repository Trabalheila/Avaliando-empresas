...
async function handleProfileReminder(req, res) {
  ...
  try {
    ...
    if (!adminUid || decoded.uid !== adminUid) return res.status(403).json({ error: "Acesso restrito ao administrador." });
    ...
    const supporters = await Promise.all(ids.map(async (id) => {
    ...
    const results = await Promise.allSettled(recipients.map((supporter) => {
