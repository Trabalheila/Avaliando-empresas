...
//
// Gera um JWT de verificação de e-mail e envia por e-mail (Resend).
// O link clicado pelo usuário aciona /api/verify-email?token=...
//
// Variáveis de ambiente:
//   EMAIL_VERIFICATION_SECRET  (obrigatória) — segredo para assinar o JWT.
//   RESEND_API_KEY             (obrigatória) — chave da Resend.
//   EMAIL_FROM_ADDRESS         (obrigatória) — remetente verificado na Resend.
//   APP_BASE_URL               (obrigatória) — base URL pública (https://www.trabalheila.com.br).
//
// Body (POST JSON): { userId: string, email: string }
...
export default async function handler(req, res) {
  ...
  if (!secret) {
    console.error('[send-verification] EMAIL_VERIFICATION_SECRET ausente');
    return res.status(500).json({ ok: false, error: 'Servidor de verificação não configurado.' });
  }
  if (!resendKey || !fromAddress) {
    console.error('[send-verification] RESEND_API_KEY ou EMAIL_FROM_ADDRESS ausente');
    return res.status(500).json({ ok: false, error: 'Serviço de e-mail não configurado.' });
  }
  if (!appBaseUrl) {
    console.error('[send-verification] APP_BASE_URL ausente');
  ...
  const html = `
    <div style="font-family:Arial,Helvetica,sans-serif;max-width:560px;margin:0 auto;color:#0f172a;">
      ...
      <p>Olá! Para concluir o cadastro no <strong>Trabalhei Lá</strong>, confirme que este e-mail é seu clicando no botão abaixo.</p>
  ...
  try {
    ...
    if (error) {
      console.error('[send-verification] Resend erro:', error);
      return res.status(502).json({ ok: false, error: 'Falha ao enviar e-mail de verificação.' });
    }
    return res.status(200).json({ ok: true });
  } catch (err) {
    console.error('[send-verification] erro inesperado:', err?.message || err);
