...
const LoginLinkedInButton = ({ clientId, redirectUri, onLoginSuccess, onLoginFailure, disabled }) => {
  ...
  const handleLogin = () => {
    ...
    // Simulação de login (você pode substituir por OAuth real depois)
    ...
    // Monitora o fechamento da popup
    const interval = setInterval(() => {
      if (popup.closed) {
        clearInterval(interval);
        // Simula sucesso (você substituirá por lógica real)
        if (onLoginSuccess) {
  ...
  return (
    <button
      onClick={handleLogin}
      disabled={disabled}
      style={{ padding: '30px 50px', fontSize: '1.125rem' }}
      className={`
        px-4 py-2 rounded-lg font-semibold text-sm transition-all flex items-center justify-center gap-2
        ${disabled 
          ? 'bg-gray-400 cursor-not-allowed opacity-60 text-white' 
          : 'bg-[#0077B5] hover:bg-[#005582] text-white hover:shadow-lg'}
      `}
    >
      <svg width={80} height={80} className="w-20 h-20" fill="currentColor" viewBox="0 0 24 24">
