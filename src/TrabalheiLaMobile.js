...
function CommentTextarea({
  ...
  return (
    <>
      ...
      />
      {showWarning && (
        <p className="text-xs text-yellow-700 dark:text-yellow-300 bg-yellow-100/80 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-700 rounded-md px-2 py-1 mt-1">
          {warningText}
        </p>
      )}
...
function TrabalheiLaMobile({
  ...
  commentDiscriminacao, setCommentDiscriminacao,
  commentCargaHoraria, setCommentCargaHoraria,
  commentCrescimento, setCommentCrescimento,
      ...
      <div className="flex flex-col w-full mt-2">
        ...
        {isRequired && (
          <p className="text-xs text-rose-600 dark:text-rose-400 mb-1">
            * Comentário obrigatório ao informar "Sim".
          </p>
        )}
        <CommentTextarea
          guidanceText={COMMENT_GUIDANCE_TEXT}
          warningText={COMMENT_WARNING_TEXT}
          containsPossiblePersonName={containsPossiblePersonName}
          className={`w-full p-2 border rounded-lg text-sm text-slate-700 dark:text-slate-100 bg-white dark:bg-slate-900 focus:outline-none focus:ring-1 mt-1 ${
            showError
  ...
  return (
    ...
    >
      ...
      <main className="px-4 space-y-3">
          ...
          {isAuthenticated ? (
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0">
                ...
                {/*
                  Botão "Painel Empresa" removido do cabeçalho mobile para
                  manter consistência com a versão desktop (que não exibe
                  esse atalho no header). O acesso ao painel da empresa
                  segue disponível pelo card "Bem-vindo(a)" do Hero
                  (CENÁRIO 3) e pelo link "Para Empresas" no rodapé.
                */}
                {(() => {
                  try {
          ...
          return (
            ...
            >
              ...
              */}
              ...
              <div className="w-16 h-1 mx-auto mb-4 rounded-full bg-gradient-to-r from-amber-300 via-amber-400 to-amber-300 shadow-[0_0_12px_rgba(251,191,36,0.6)]" />
              ...
              {!isAuthenticated && (
                <>
                  ...
                  {/* Login social secundário — promovido a "Cadastre-se em 10s"
                      para reduzir abandono. Mantém função de login (mesma
                      ação do botão vale para usuários novos e existentes). */}
                  <div className="pt-3 border-t border-white/15">
                    <p className="text-[11px] uppercase tracking-wider text-amber-200 text-center mb-1 font-extrabold">
                      ⚡ Cadastre-se em 10 segundos
                    </p>
                    <p className="text-[11px] text-blue-100/90 text-center mb-2">
                      Sem formulários. Sem senha. Sua identidade fica oculta.
                    </p>
                    <div className="flex flex-col gap-2">
                      <button
                        ...
                        disabled={isLoading}
                        className="w-full inline-flex items-center justify-center gap-2 bg-white hover:bg-blue-50 text-slate-800 font-bold py-2.5 px-3 rounded-xl shadow-md hover:shadow-lg transition-all text-sm disabled:opacity-60"
                      >
              ...
              {(!isAuthenticated || !hasDefinedProfileType) && (
                <div className="mt-4 pt-3 border-t border-white/15 flex flex-wrap items-center justify-center gap-1.5">
                  <span
                    ...
                    className="inline-flex items-center gap-1 bg-white/10 text-white px-2 py-0.5 rounded-full text-[10px] font-semibold"
                  >
                    ...
                    {isSpecialistProfile ? (
                      <>
                        <button
                          ...
                          className="inline-flex items-center justify-center min-h-[40px] px-3 py-2 rounded-lg bg-white text-blue-800 text-xs font-bold shadow hover:bg-blue-50 transition"
                        >
                          Meu Painel
                        </button>
                        <button
                          ...
                          className="inline-flex items-center justify-center min-h-[40px] px-3 py-2 rounded-lg bg-transparent border-2 border-white text-white text-xs font-bold hover:bg-white/10 transition"
                        >
                          Meu Perfil Público
                        </button>
                      </>
              ...
              <div className="mt-5 pt-4 border-t border-white/20 flex justify-center">
                ...
                >
                  ...
                  <span className="flex-1 text-center">Premium para Trabalhadores e Especialistas</span>
                  <span aria-hidden="true" className="flex-shrink-0">›</span>
                </button>
              </div>
            </section>
        ...
        <section className="mt-6 bg-white dark:bg-slate-900 rounded-2xl shadow-md p-5 border border-blue-100 dark:border-slate-700">
          <h3 className="text-sm uppercase tracking-[0.14em] font-extrabold text-blue-800 dark:text-blue-200 text-center mb-2">
            Como funciona?
          </h3>
        ...
        {/* FORMULÁRIO */}
        <section className="mt-6 bg-white dark:bg-slate-900 rounded-2xl shadow-md p-5 border border-blue-50 dark:border-slate-700">
            ...
            <div>
              ...
              {showNewCompanyInput && (
                <div className="mt-3 space-y-2">
                  ...
                  {isUserAdmin && (
                    <div className="rounded-xl border border-dashed border-blue-200 dark:border-slate-700 p-3 bg-blue-50/60 dark:bg-slate-800/60 mt-2">
                  ...
                  {pendingCompanyData && (
                    <div className="bg-blue-50 dark:bg-slate-800 border border-blue-200 dark:border-slate-700 rounded-xl p-3 text-sm text-blue-900 dark:text-blue-100">
                      ...
                      <p className="text-xs text-blue-700 mt-1">CNPJ: {pendingCompanyData.cnpj || "não informado"}</p>
                      {pendingCompanyData.razaoSocial && (
                        <p className="text-xs text-blue-700 mt-0.5">Razão social: {pendingCompanyData.razaoSocial}</p>
                      )}
                      {pendingCompanyData.segmento && (
                        <p className="text-xs text-indigo-700 mt-0.5">Segmento (CNAE): {pendingCompanyData.segmento}</p>
                      )}
