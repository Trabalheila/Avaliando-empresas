...
export default function MinhaConta({ theme, toggleTheme }) {
    ...
    async function load(uid) {
      ...
      try {
        ...
        if (!userSnap.exists() && uid && uid !== profileId) {
          ...
          if (altSnap.exists()) { userSnap = altSnap; resolvedId = uid; }
        }
        ...
        if (!userSnap.exists() && !apoiadorData && !unifiedByEmail && !stored?.pseudonimo && !stored?.pseudonym && !stored?.email) {
          if (!cancelled) {
            ...
            setProfile({
              id: cu.uid || uid, uid: cu.uid || uid, email: cu.email || "",
              pseudonimo: "", avatar: cu.photoURL || "", picture: cu.photoURL || "",
              photoURL: cu.photoURL || "", nomeReal: cu.displayName || "", fullName: cu.displayName || "",
            });
            setLoading(false);
          }
          return;
        ...
        const userData = userSnap.exists()
          ...
          : apoiadorData
            ? { ...apoiadorData, pseudonimo: apoiadorData.pseudonimo || apoiadorData.nome || stored?.pseudonym || "", userType: "apoiador", isApoiador: true }
            : unifiedByEmail
              ? { ...unifiedByEmail }
        ...
        for (const snap of reviewSnaps) {
          for (const d of snap.docs) {
            if (!reviewsById.has(d.id)) reviewsById.set(d.id, { id: d.id, ...d.data() });
          }
        }
  ...
  // ── Seleciona arquivo e gera preview (sem upload) ──
  const handleAvatarFile = useCallback(async (event) => {
    ...
    if (file.size > 10 * 1024 * 1024) {
      setAvatarError("Imagem muito grande (máximo 10MB).");
      return;
    }
    try {
      const compressed = await compressImage(file, 256, 0.82);
      setPendingFile(compressed?.blob || file);
      setPendingPreview(compressed?.dataUrl || "");
    } catch {
      setPendingFile(file);
      const reader = new FileReader();
      reader.onload = (e) => setPendingPreview(String(e.target?.result || ""));
      reader.readAsDataURL(file);
    }
  }, []);

  // ── Faz o upload de fato ao clicar em Salvar ──
  const handleSaveAvatar = useCallback(async () => {
    if (!pendingFile) return;
    setAvatarError("");
    const currentUid = auth.currentUser?.uid || "";
    const docId = profile?.id || currentUid;
    const storageUid = currentUid || docId;
    if (!docId || !storageUid) {
      setAvatarError("Perfil não identificado. Faça login novamente.");
      return;
    }
    try { if (auth.currentUser) await auth.currentUser.getIdToken(true); } catch { /* ignore */ }
    setUploadingAvatar(true);
    setUploadProgress(0);
    const fallbackDataUrl = pendingPreview;

    const persistAvatar = async (url) => {
      const cu = auth.currentUser || {};
      const writeIds = [...new Set([docId, storageUid].filter(Boolean))];
      let saved = false;
      ...
      if (!saved) throw new Error("Sem permissão para salvar.");
      setProfile((prev) => (prev ? { ...prev, avatar: url, photoURL: url } : prev));
      try {
        const stored = JSON.parse(localStorage.getItem("userProfile") || "{}");
        localStorage.setItem("userProfile", JSON.stringify({ ...stored, avatar: url, photoURL: url }));
    ...
    try {
      ...
      const task = uploadBytesResumable(ref, pendingFile, { contentType: "image/jpeg" });
      await new Promise((resolve, reject) => {
        ...
        task.on("state_changed",
          (snap) => { if (snap.totalBytes > 0) setUploadProgress(Math.round((snap.bytesTransferred / snap.totalBytes) * 100)); },
      ...
      if (/timeout/i.test(code)) msg = "O envio demorou demais. Tente novamente.";
      else if (/unauthorized|permission/i.test(code)) msg = "Sem permissão. Faça login novamente.";
      else if (/quota/i.test(code)) msg = "Cota excedida. Tente mais tarde.";
      else if (/network|retry-limit|canceled/i.test(code)) msg = "Falha de rede. Tente novamente.";
      setAvatarError(msg);
    } finally {
      setUploadingAvatar(false);
      setUploadProgress(0);
    }
  }, [pendingFile, pendingPreview, profile]);

  // ── Remove a foto atual ou cancela o preview pendente ──
  const handleRemoveAvatar = useCallback(async () => {
    if (pendingFile) { setPendingFile(null); setPendingPreview(""); return; }
    const currentUid = auth.currentUser?.uid || "";
    const docId = profile?.id || currentUid;
    if (!docId) return;
    try {
      const cu = auth.currentUser || {};
      const writeIds = [...new Set([docId, currentUid].filter(Boolean))];
      for (const id of writeIds) {
        try {
          await setDoc(doc(db, "users", id),
            { avatar: "", photoURL: "", ...(cu.uid ? { uid: cu.uid } : {}), updatedAt: serverTimestamp() },
            { merge: true }
          );
          break;
        } catch { continue; }
      ...
      try {
        const stored = JSON.parse(localStorage.getItem("userProfile") || "{}");
        localStorage.setItem("userProfile", JSON.stringify({ ...stored, avatar: "", photoURL: "" }));
        window.dispatchEvent(new Event("trabalheiLa_user_updated"));
      } catch { /* ignore */ }
    } catch { setAvatarError("Não foi possível remover a foto."); }
  }, [pendingFile, profile]);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 dark:from-slate-950 dark:to-slate-900 flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-3">
          <div aria-hidden="true" className="h-10 w-10 rounded-full border-2 border-blue-200 dark:border-slate-700 border-t-blue-600 dark:border-t-blue-400 animate-spin" />
  ...
  const hasCurrentAvatar = !!(safeProfile?.avatar || safeProfile?.photoURL || safeProfile?.picture);
      ...
      <div className="flex-1 w-full max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <section className="bg-white dark:bg-slate-900 rounded-3xl shadow-xl p-6 sm:p-8 border border-blue-100 dark:border-slate-700">
          <div className="flex flex-col sm:flex-row items-center gap-5">
            ...
            <div className="flex flex-col items-center gap-2">
              {pendingPreview ? (
                <img src={pendingPreview} alt="preview" className="h-20 w-20 rounded-full object-cover border-2 border-blue-400" />
              ) : (
                avatarDisplay
              )}
              <input ref={avatarInputRef} type="file" accept="image/*" onChange={handleAvatarFile} className="hidden" />

              {!pendingFile && !uploadingAvatar && (
              ...
              {pendingFile && !uploadingAvatar && (
                <div className="flex gap-2">
                  <button type="button" onClick={handleSaveAvatar}
                    className="text-xs font-semibold px-3 py-1 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition">
                    Salvar
                  </button>
                  <button type="button" onClick={handleRemoveAvatar}
                    className="text-xs font-semibold px-3 py-1 rounded-lg bg-red-100 text-red-600 hover:bg-red-200 transition">
                    Cancelar
                  </button>
                </div>
              )}
              ...
              {uploadingAvatar && (
                <>
                  ...
                  <div className="w-28 h-1.5 rounded-full bg-blue-100 dark:bg-slate-700 overflow-hidden"
                    role="progressbar" aria-valuenow={uploadProgress} aria-valuemin={0} aria-valuemax={100}>
                    <div className="h-full bg-blue-600 dark:bg-blue-400 transition-all duration-200"
              ...
              {!pendingFile && !uploadingAvatar && hasCurrentAvatar && (
                <button type="button" onClick={handleRemoveAvatar}
            ...
            <div className="flex-1 text-center sm:text-left">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Membro desde {memberSince}</p>
              {safeProfile?.email && (
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">{safeProfile.email}</p>
              )}
              {credibility && credibility !== "—" && (
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">
                  Índice de credibilidade:{" "}
                  <span className="font-semibold capitalize">
                    {credibility === "confiavel" ? "Confiável" : credibility === "neutro" ? "Neutro" : credibility === "atencao" ? "Atenção" : credibility}
                  </span>
                </p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-2 shrink-0">
              <button type="button" onClick={() => setEditProfileOpen(true)}
              ...
              <button type="button" onClick={handleViewPublicProfile}
        ...
        <section className="bg-white dark:bg-slate-900 rounded-3xl shadow-xl p-6 sm:p-8 border border-blue-100 dark:border-slate-700">
          ...
          {!isPremium() && (
            <div className="mt-4 flex flex-wrap gap-2">
              <button type="button" onClick={() => navigate("/escolha-perfil?planos=1")}
                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 transition">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <div className="relative inline-flex items-center group">
                <button type="button" onClick={() => setConsultaIntroOpen(true)}
                ...
                <span tabIndex={0} role="button" aria-label="Sobre Consulta Avulsa"
                  ...
                  <span role="tooltip" className="pointer-events-none absolute left-1/2 top-full mt-2 -translate-x-1/2 z-20 w-64 max-w-[80vw] px-3 py-2 rounded-xl bg-slate-900 text-white text-xs leading-snug shadow-xl opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition">
          ...
          {!isPremium() && (
            <p className="mt-3 text-xs text-slate-500 dark:text-slate-400">
              <strong>Consulta Avulsa</strong> é uma interação pontual com um especialista (sujeita a aceite do profissional). A <strong> consulta com acompanhamento</strong> e a escolha avançada de profissionais são exclusivas do Plano Premium.
            </p>
          )}
        </section>
        ...
        <section className="bg-white dark:bg-slate-900 rounded-3xl shadow-xl p-6 sm:p-8 border border-blue-100 dark:border-slate-700">
          ...
          <p className="text-sm text-slate-600 dark:text-slate-300 mb-4">
            Conecte sua conta do LinkedIn para dar mais credibilidade às suas avaliações. Quando o nome de uma empresa avaliada corresponder a uma experiência do seu perfil, a avaliação recebe o selo{" "}
            <span className="font-semibold text-emerald-700 dark:text-emerald-400">✓ Profissional Verificado via LinkedIn</span>.
          ...
          {isLinkedInConnected(safeProfile) ? (
            <div className="flex items-center gap-3 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-700 px-4 py-3">
              ...
              <div>
                <p className="text-sm font-semibold text-emerald-700 dark:text-emerald-300">LinkedIn conectado</p>
                <p className="text-xs text-emerald-600 dark:text-emerald-400">Suas avaliações são validadas automaticamente pelo seu histórico profissional.</p>
              </div>
            </div>
          ) : !linkedInClientId ? (
            <p className="text-sm text-amber-600 dark:text-amber-400">A integração com o LinkedIn não está configurada no momento.</p>
          ) : (
            <div className="max-w-xs">
              <LoginLinkedInButton
                clientId={linkedInClientId}
                redirectUri={linkedInRedirectUri}
                onLoginSuccess={({ code } = {}) => { if (code) navigate(`/?linkedin_code=${encodeURIComponent(code)}`); }}
                onLoginFailure={(err) => setLinkedInError(err?.message || "Falha ao conectar com o LinkedIn.")}
              />
              {linkedInError && <p className="text-red-600 text-xs mt-2">{linkedInError}</p>}
            </div>
          )}
        </section>

        <ConsultaAvulsaIntroModal open={consultaIntroOpen} onClose={() => setConsultaIntroOpen(false)}
          onContinue={() => { setConsultaIntroOpen(false); setConsultaAvulsaOpen(true); }} />
        <ConsultaAvulsaModal open={consultaAvulsaOpen} onClose={() => setConsultaAvulsaOpen(false)} worker={safeProfile} />
        <EditProfileModal open={editProfileOpen} onClose={() => setEditProfileOpen(false)} profile={safeProfile} onSaved={(next) => setProfile(next)} />
        ...
        <div className="lg:columns-2 lg:gap-6 [&>*]:break-inside-avoid [&>*]:mb-6">
          ...
          <section id="experiencias" className="bg-white dark:bg-slate-900 rounded-3xl shadow-xl p-6 sm:p-8 border border-blue-100 dark:border-slate-700">
            <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
              ...
              <button type="button" onClick={() => setExperienceModalOpen(true)}
            ...
            {experiences.length === 0 ? (
              <p className="text-slate-500 dark:text-slate-400 text-sm text-center py-6">Nenhuma experiência profissional adicionada.</p>
            ) : (
                ...
                {experiences.map((exp, i) => {
                  ...
                  return (
                    <li key={`${exp.company}-${exp.role}-${i}`}
                      ...
                      {isVerified ? (
                        <span className="inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-700" title="Experiência verificada via LinkedIn">
                        ...
                        <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-600" title="Adicionada manualmente — não verificada">
                          Não verificada
                        </span>
                      )}
                    </li>
          ...
          <ExperienceManagerModal open={experienceModalOpen} onClose={() => setExperienceModalOpen(false)} profile={safeProfile} onSaved={(next) => setProfile(next)} />
          ...
          <section className="bg-white dark:bg-slate-900 rounded-3xl shadow-xl p-6 sm:p-8 border border-blue-100 dark:border-slate-700">
            ...
            {reviews.length === 0 ? (
              <p className="text-slate-500 dark:text-slate-400 text-sm text-center py-6">Você ainda não avaliou nenhuma empresa.</p>
            ) : (
              <div className="space-y-3">
                {reviews.map((review) => (
                  <button key={review.id} type="button"
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-slate-800 dark:text-slate-100 truncate">{review.company || review.companySlug}</p>
                    </div>
                    <div className="flex items-center gap-3 shrink-0">
                      <div className="flex items-center gap-1">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <span key={star} className={`text-sm ${star <= Number(review.rating) ? "text-amber-400" : "text-slate-300 dark:text-slate-600"}`}>★</span>
                        ))}
                      </div>
                      <span className="text-xs text-slate-400 dark:text-slate-500 whitespace-nowrap">{formatDate(review.createdAt)}</span>
                    </div>
                  </button>
                ))}
              </div>
          ...
          <section className="bg-white dark:bg-slate-900 rounded-3xl shadow-xl p-6 sm:p-8 border border-blue-100 dark:border-slate-700">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <button type="button" onClick={() => navigate("/excluir-dados")}
    ...
    (async () => {
      try {
        const [snap, casos] = await Promise.all([
          getDocs(collection(db, "users", uid, "releasedContacts")).catch(() => ({ docs: [] })),
        ...
        for (const c of released) {
          ...
          if (c.especialidade && !entry.specialtyId) entry.specialtyId = c.especialidade;
    ...
    <section className="bg-white dark:bg-slate-900 rounded-3xl shadow-xl p-6 sm:p-8 border border-blue-100 dark:border-slate-700">
      ...
      {loading ? (
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Nenhum contato liberado ainda. Ao contratar uma consulta ou ter um pedido aceito por um especialista, ele aparece aqui — uma entrada por especialista, com todos os seus casos reunidos.
        </p>
      ) : (
        <ul className="space-y-3">
          {specialists.map((s) => {
            ...
            return (
              <li key={s.especialistaId} className="p-4 rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60">
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div className="min-w-0">
                    <p className="text-sm font-bold text-slate-800 dark:text-slate-100 break-words">{s.nomeDoEspecialista}</p>
                    {s.specialtyId && <p className="text-xs text-slate-500 dark:text-slate-400">⚖️ {s.specialtyId}</p>}
                    {s.casosCount > 0 && (
                  ...
                  <button type="button"
                    onClick={() => navigate(`/trabalhador/especialista/${encodeURIComponent(s.especialistaId)}/casos`)}
...
function ActiveServicesSection({ profile, navigate }) {
  ...
  return (
    <section className="bg-white dark:bg-slate-900 rounded-3xl shadow-xl p-6 sm:p-8 border border-blue-100 dark:border-slate-700">
      ...
      {loading ? (
        <p className="text-sm text-slate-500 dark:text-slate-400 animate-pulse">Carregando atendimentos…</p>
      ) : groups.length === 0 ? (
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Você ainda não tem atendimentos ativos. Quando um especialista aceitar o seu pedido, cada caso aparecerá aqui — agrupado por especialista — com uma página dedicada para acompanhar e enviar documentos.
        </p>
      ) : (
          ...
          {groups.map((g) => (
            <div key={g.especialistaId}>
              <div className="flex items-center justify-between gap-2 mb-2">
                <div className="min-w-0">
                  <p className="text-sm font-bold text-slate-800 dark:text-slate-100 break-words">{g.nomeDoEspecialista}</p>
                  {g.specialtyId && <p className="text-xs text-slate-500 dark:text-slate-400">⚖️ {g.specialtyId}</p>}
                </div>
              ...
              <ul className="space-y-2">
                {g.casos.map((c, idx) => (
                  <li key={c.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60">
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-slate-800 dark:text-slate-100 break-words">{casoLabel(c, idx)}</p>
                      <p className="text-xs text-emerald-700 dark:text-emerald-300 font-semibold mt-0.5">
      ...
      {!workerIsPremium ? (
        <div className="rounded-2xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 p-4">
          <p className="text-sm text-slate-700 dark:text-slate-200">
            🔒 Acesso a videochamadas é um benefício exclusivo do <strong>Plano Premium</strong>. Faça upgrade para se conectar com seu especialista por vídeo.
          </p>
          <button type="button" onClick={() => navigate("/trabalhador/beneficios")}
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 animate-pulse">Carregando agenda…</p>
      ) : consultas.length === 0 ? (
        <p className="text-sm text-slate-500 dark:text-slate-400">Nenhuma videochamada agendada no momento.</p>
      ) : (
        <ul className="space-y-3">
          {consultas.map((c) => {
            ...
            return (
              <li key={c.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60">
                <div className="min-w-0">
                  ...
                  {when && <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{when}</p>}
                  {startsIn && <p className="text-xs font-semibold text-emerald-700 dark:text-emerald-300 mt-0.5">{startsIn}</p>}
                </div>
