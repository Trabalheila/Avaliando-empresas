...
export default function PagamentoConsultaPage({ theme, toggleTheme }) {
  ...
  const consultationData = useMemo(() => {
    const consultationPrice = Number(state.consultationPrice || 0) || 0;
    const originalPrice = Number(state.originalPrice || consultationPrice) || consultationPrice;
    const discountAmount = Number(state.discountAmount || Math.max(0, originalPrice - consultationPrice)) || 0;
    return {
      professionalId: state.professionalId || "",
      professionalUid: state.professionalUid || "",
      professionalName: state.professionalName || "",
      specialtyId: state.specialtyId || "outro",
      specialtyLabel: state.specialtyLabel || "Especialidade",
      consultationPrice,
      originalPrice,
      discountAmount,
      userDoubt: String(state.userDoubt || "").trim(),
    };
  }, [state]);

  const hasValidData =
    Boolean(consultationData.professionalId) &&
    Boolean(consultationData.professionalName) &&
    consultationData.consultationPrice > 0 &&
    Boolean(consultationData.userDoubt);

  async function persistConsultationAndTransaction(paymentMeta) {
    setError("");
    const workerId = auth.currentUser?.uid || "";
    if (!workerId) {
      setError("Faca login para concluir o pagamento.");
      return;
    }
    if (!hasValidData) {
      setError("Dados da consulta invalidos. Volte e tente novamente.");
      return;
    }
    ...
    try {
      const payload = {
        workerId,
        workerNome: workerProfile?.pseudonimo || workerProfile?.nome || "Trabalhador",
        apoiadorId: consultationData.professionalId,
        apoiadorUid: consultationData.professionalUid || "",
        apoiadorNome: consultationData.professionalName,
        especialidade: consultationData.specialtyId,
        tipo: "avulsa",
        type: "avulsa",
        message: consultationData.userDoubt.slice(0, 2000),
        status: "pending",
        readByApoiador: false,
        amount: consultationData.consultationPrice,
        originalAmount: consultationData.originalPrice,
        discountApplied: consultationData.discountAmount,
        platformCommission: consultationData.consultationPrice * 0.2,
        amountDueToSpecialist: consultationData.consultationPrice * 0.8,
        paymentStatus: "paid",
        createdAt: serverTimestamp(),
      };
      ...
      try {
        await registerConsultationTransaction({
          userId: workerId,
          specialistId: consultationData.professionalId,
          consultationId: ref.id,
          originalAmount: consultationData.originalPrice,
          discountApplied: consultationData.discountAmount,
          finalAmountPaid: consultationData.consultationPrice,
          platformCommission: consultationData.consultationPrice * 0.2,
          amountDueToSpecialist: consultationData.consultationPrice * 0.8,
          paymentMeta,
        });
      } catch (err) {
        console.warn("PagamentoConsultaPage: falha ao registrar transacao", err);
      }
      ...
      try {
        await fetch(buildApiUrl("/api/send-contact-request"), {
          ...
          body: JSON.stringify({
            ...
            toUid: consultationData.professionalUid || consultationData.professionalId,
            toPseudonym: consultationData.professionalName,
            message: payload.message,
            especialidade: consultationData.specialtyId,
          }),
        });
      ...
      console.warn("PagamentoConsultaPage:", err);
      setError(err?.message || "Nao foi possivel finalizar o pagamento.");
    } finally {
          ...
          {!hasValidData ? (
            <>
              <h1 className="text-xl font-extrabold text-slate-800 dark:text-slate-100">
                Erro: dados da consulta nao encontrados
              </h1>
              <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
                Volte para a tela de consulta avulsa e tente novamente.
              </p>
              <button
                type="button"
                onClick={() => navigate("/consulta-avulsa")}
                className="mt-4 px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-bold"
              >
                Ir para consulta avulsa
              </button>
            </>
          ) : success ? (
            <>
              <h1 className="text-xl font-extrabold text-emerald-700 dark:text-emerald-300">
                Pagamento confirmado
              </h1>
              <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
                Sua solicitacao de consulta com {consultationData.professionalName} foi enviada.
              </p>

              <div className="mt-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/40 p-3 text-sm">
                <div className="flex justify-between">
                  <span className="text-slate-600 dark:text-slate-300">Especialista</span>
                  <strong className="text-slate-800 dark:text-slate-100">{consultationData.professionalName}</strong>
                </div>
                <div className="flex justify-between mt-1">
                  <span className="text-slate-600 dark:text-slate-300">Valor pago</span>
                  <strong className="text-slate-800 dark:text-slate-100">{formatBRL(consultationData.consultationPrice)}</strong>
                </div>
                {consultationData.discountAmount > 0 && (
                  <div className="flex justify-between mt-1 text-xs text-emerald-700 dark:text-emerald-300">
                    <span>Desconto Premium aplicado</span>
                    <span>- {formatBRL(consultationData.discountAmount)}</span>
                  </div>
                )}
              </div>
            ...
            <>
              ...
              <div className="mt-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/40 p-3 text-sm">
                ...
                <div className="flex justify-between mt-1">
                  <span className="text-slate-600 dark:text-slate-300">Especialidade</span>
                  <strong className="text-slate-800 dark:text-slate-100">{consultationData.specialtyLabel}</strong>
                </div>
                <div className="flex justify-between mt-1">
                  <span className="text-slate-600 dark:text-slate-300">Valor</span>
                  <strong className="text-slate-800 dark:text-slate-100">
                    {consultationData.discountAmount > 0 ? (
                      <>
                        <span className="line-through text-slate-400 dark:text-slate-500 mr-2 font-normal">
                          {formatBRL(consultationData.originalPrice)}
                        </span>
                        {formatBRL(consultationData.consultationPrice)}
                      </>
                    ) : (
                      formatBRL(consultationData.consultationPrice)
                    )}
              ...
              <p className="mt-3 text-xs text-slate-500 dark:text-slate-400">
                Duvida enviada ao profissional apos pagamento: {consultationData.userDoubt}
              </p>

              <div className="mt-4">
                <StripePaymentForm
                  amount={consultationData.consultationPrice}
                  currencyLabel={formatBRL(consultationData.consultationPrice)}
                  onPaymentSuccess={(paymentMeta) => persistConsultationAndTransaction(paymentMeta)}
                  onCancel={() => navigate(-1)}
