...
function CompanyDetails({ theme, toggleTheme }) {
  ...
  const [insightsLoading, setInsightsLoading] = React.useState(false);
    ...
    const [companyReplyTargetId, setCompanyReplyTargetId] = React.useState(null);
    const [companyReplyText, setCompanyReplyText] = React.useState("");
    const [companyReplySaving, setCompanyReplySaving] = React.useState(false);
        ...
        const normalized = snap.docs.map((d) => {
          ...
          return {
            ...
            replies: Array.isArray(data.replies)
              ...
              : [],
            companyReply: data.companyReply || null,
          };
        });
  ...
  // ─── Resposta oficial da empresa a um comentário ───
  const handleCompanyReply = async (commentId) => {
    const text = companyReplyText.trim();
    if (!text || !commentId) return;
    setCompanyReplySaving(true);
    try {
      const companySlug = getCompanySlug();
      if (!companySlug) throw new Error("Empresa não identificada.");

      // Buscar o documento no Firestore para obter a referência correta
      const ref = collection(db, "comments");
      const q = query(ref, where("companySlug", "==", companySlug), limit(200));
      const snap = await getDocs(q);
      const targetDoc = snap.docs.find((d) => d.id === commentId);

      if (!targetDoc) throw new Error("Comentário não encontrado.");
      ...
      await updateDoc(targetDoc.ref, { companyReply: replyData });

      // Atualizar localmente
      const next = comments.map((c) =>
        c.id === commentId ? { ...c, companyReply: replyData } : c
      );
      saveComments(next);

      setCompanyReplyTargetId(null);
      setCompanyReplyText("");
      setActionNotice("Resposta enviada. Ela ficará visível após aprovação pela moderação.");
    } catch (err) {
      console.warn("Erro ao salvar resposta da empresa:", err);
      setActionNotice("Erro ao enviar resposta. Tente novamente.");
    } finally {
      setCompanyReplySaving(false);
    }
  };
              ...
              [...visibleComments]
                ...
                .map((comment) => (
                  <div key={comment.id} className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-gray-200 dark:border-slate-700">
                    ...
                    {/* ── Resposta oficial da empresa (aprovada) ── */}
                    {comment.companyReply && comment.companyReply.status === "approved" && (
                      <div className="mt-4 rounded-xl border-l-4 border-blue-600 dark:border-blue-400 bg-blue-50 dark:bg-blue-950/30 p-4">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-sm font-bold text-blue-700 dark:text-blue-300">Resposta oficial da empresa</span>
                        </div>
                        <p className="text-sm text-slate-800 dark:text-slate-100 whitespace-pre-line">{comment.companyReply.text}</p>
                        {comment.companyReply.publishedAt && (
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">
                            {new Date(comment.companyReply.publishedAt).toLocaleDateString("pt-BR", { day: "2-digit", month: "short", year: "numeric" })}
                          </p>
                        )}
                      </div>
                    )}

                    {/* ── Resposta pendente (visível apenas para o gestor) ── */}
                    {comment.companyReply && comment.companyReply.status === "pending" && isEmpresaPremium && (
                      <div className="mt-4 rounded-xl border border-amber-200 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/20 p-3">
                        <p className="text-xs font-semibold text-amber-700 dark:text-amber-400 mb-1">Resposta enviada — aguardando aprovação da moderação</p>
                        <p className="text-sm text-slate-700 dark:text-slate-300 whitespace-pre-line">{comment.companyReply.text}</p>
                      </div>
                    )}

                    {/* ── Botão/Formulário de resposta da empresa ── */}
                    {isEmpresaPremium && !comment.companyReply && companyReplyTargetId !== comment.id && (
                      <div className="mt-3">
                        <button
                          type="button"
                          onClick={() => { setCompanyReplyTargetId(comment.id); setCompanyReplyText(""); }}
                          className="inline-flex items-center gap-1.5 text-xs font-bold text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 hover:underline transition"
                        >
                    ...
                    {isEmpresaPremium && companyReplyTargetId === comment.id && (
                      <div className="mt-3 rounded-xl border border-blue-200 dark:border-blue-700 bg-blue-50/50 dark:bg-blue-950/20 p-4">
                        <p className="text-xs font-bold text-blue-700 dark:text-blue-300 mb-2">Resposta oficial da empresa</p>
                        <textarea
                          value={companyReplyText}
                          onChange={(e) => { if (e.target.value.length <= 500) setCompanyReplyText(e.target.value); }}
                          placeholder="Escreva a resposta oficial da empresa..."
                          className="w-full p-3 border border-blue-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                          rows={3}
                          maxLength={500}
                        />
                        <div className="flex items-center justify-between mt-2">
                          <span className="text-xs text-slate-400 dark:text-slate-500">{companyReplyText.length}/500</span>
                          <div className="flex gap-2">
                            <button
                              type="button"
                              onClick={() => { setCompanyReplyTargetId(null); setCompanyReplyText(""); }}
                              className="px-3 py-1.5 text-xs font-semibold text-slate-600 dark:text-slate-300 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 transition"
                            >
                              Cancelar
                            </button>
                            <button
                              type="button"
                              onClick={() => handleCompanyReply(comment.id)}
                              disabled={!companyReplyText.trim() || companyReplySaving}
                              className="px-4 py-1.5 text-xs font-bold rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                              {companyReplySaving ? "Publicando…" : "Publicar resposta"}
                            </button>
                          </div>
                        </div>
                      </div>
