...
function TrabalheiLa() {
  ...
  const [companies, setCompanies] = useState([
    'Banco do Brasil', 'Raízen Combustíveis', 'Itaú Unibanco Holding',
    'Grupo Raízen', 'Bradesco', 'Vale', 'Itaú Unibanco',
    'Caixa Econômica Federal', 'Grupo Carrefour Brasil', 'Magazine Luiza',
    'Ambev', 'Embraer', 'WEG', 'Suzano Papel e Celulose', 'XP Inc.',
  ...
  const handleGoogleLogin = () => {
    setIsLoading(true);
    // Aqui você implementará a lógica real do Google OAuth
    setTimeout(() => {
  ...
  const handleSubmit = (e) => {
    ...
    const novaAvaliacao = {
      company: company.value, rating, contatoRH, salarioBeneficios,
      estruturaEmpresa, acessibilidadeLideranca, planoCarreiras,
      bemestar, estimulacaoOrganizacao,
      comments: {
        rating: commentRating, contatoRH: commentContatoRH,
      ...
      comment, area: 'Tecnologia', periodo: '2021-2024',
    };

    setEmpresas([novaAvaliacao, ...empresas]);
    setCompany(null); setRating(0); setComment('');
    setContatoRH(0); setSalarioBeneficios(0); setEstruturaEmpresa(0);
    setAcessibilidadeLideranca(0); setPlanoCarreiras(0); setBemestar(0);
  ...
  const calcularMedia = (emp) => (
    (emp.rating + emp.contatoRH + emp.salarioBeneficios + emp.estruturaEmpresa +
     emp.acessibilidadeLideranca + emp.planoCarreiras + emp.bemestar + emp.estimulacaoOrganizacao) / 8
  ).toFixed(1);
  ...
  return (
    <div className="min-h-screen p-2 md:p-8" style={{ backgroundImage: 'url("/fundo-new.png")', backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed' }}>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 md:gap-6">
              <div className="text-center md:text-left">
                <h1 className="text-3xl md:text-5xl font-black text-white tracking-tight drop-shadow-[0_0_10px_rgba(0,0,0,0.9)]">
                  Trabalhei <span className="text-[#4FC3F7] drop-shadow-[0_0_8px_rgba(0,0,0,1)]">Lá</span>
                </h1>
                <p className="mt-2 md:mt-3 text-xs md:text-base font-bold text-white">Avaliações reais, anônimas e confiáveis.</p>
              </div>
              {isAuthenticated && (
          ...
          <div className="bg-white/90 backdrop-blur-lg rounded-3xl shadow-2xl p-3 md:p-8 border border-white/20">
                ...
                <div className="flex items-start gap-3 md:gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs md:text-sm text-blue-50">
                      Usamos o LinkedIn ou Google apenas para verificar seu vínculo profissional. Suas avaliações são <strong>100% anônimas</strong> — nome e perfil nunca são exibidos.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
              <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-4 md:p-6 border border-gray-200">
                ...
                <Select value={company} onChange={setCompany} options={companyOptions} formatOptionLabel={formatOptionLabel} placeholder="Digite ou selecione..." className="mb-3 md:mb-4 text-sm" />
                <div className="flex gap-2">
                  <input type="text" value={newCompany} onChange={(e) => setNewCompany(e.target.value)} className="flex-1 border-2 border-gray-300 p-2 md:p-3 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent text-sm" placeholder="Ou adicione uma nova empresa" />
                  <button type="button" onClick={handleAddCompany} className="bg-gradient-to-r from-green-500 to-emerald-600 text-white px-3 md:px-4 py-2 rounded-xl hover:shadow-lg transition-all font-semibold whitespace-nowrap text-xs md:text-sm">Adicionar</button>
                </div>
              </div>
                  ...
                  return (
                    <div key={idx} className="bg-white rounded-xl p-3 md:p-4 border-2 border-gray-200 hover:border-purple-400 transition-all">
                      ...
                      <div className="flex gap-1 mb-2 md:mb-3">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <FaStar key={star} size={20} className="cursor-pointer transition-all hover:scale-110" color={star <= item.value ? '#facc15' : '#e5e7eb'} onClick={() => item.setter(star)} />
                        ))}
                      </div>
                      <textarea value={item.comment} onChange={(e) => item.setComment(e.target.value)} rows={2} className="w-full border border-gray-300 p-2 rounded-lg text-xs md:text-sm focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none" placeholder={`Comente sobre ${item.label.toLowerCase()} (opcional)`} />
                    </div>
                  );
                })}
              </div>
              ...
              <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-4 md:p-6 border-2 border-purple-200">
                ...
                <textarea value={comment} onChange={(e) => setComment(e.target.value)} rows={4} className="w-full border-2 border-purple-300 p-3 md:p-4 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none text-xs md:text-sm" placeholder="Compartilhe uma visão geral sobre sua experiência (opcional)" />
              </div>

              <div className="flex flex-col items-center space-y-3 md:space-y-4">
                {!isAuthenticated ? (
                  <div className="w-full max-w-xs space-y-3">
                    <LoginLinkedInButton clientId={process.env.REACT_APP_LINKEDIN_CLIENT_ID || '77dv5urtc8ixj3'} redirectUri="https://www.trabalheila.com.br/auth/linkedin" onLoginSuccess={handleLinkedInSuccess} onLoginFailure={handleLinkedInFailure} disabled={isLoading} />
                    ...
                    {isLoading && <p className="text-xs md:text-sm text-gray-600 mt-2 md:mt-3 text-center animate-pulse">Autenticando...</p>}
                  </div>
                ) : (
                ...
                <button type="submit" className={`px-6 md:px-8 py-2.5 md:py-3 rounded-xl text-white font-semibold text-xs md:text-base transition-all max-w-xs w-full ${isAuthenticated ? 'bg-gradient-to-r from-blue-600 to-purple-600 hover:shadow-lg' : 'bg-gray-400 cursor-not-allowed opacity-60'}`} disabled={!isAuthenticated}>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white/90 backdrop-blur-lg rounded-3xl shadow-2xl p-4 md:p-6 border border-white/20 lg:sticky lg:top-8">
            <div className="flex flex-col items-center mb-4 md:mb-6">
              <h2 className="text-sm md:text-base font-bold text-slate-700 text-center mb-3">Ranking - Top Empresas Avaliadas</h2>
              <img src="/trofeu-new.png" alt="Troféu Trabalhei Lá" className="w-16 h-16 md:w-20 md:h-20 object-contain drop-shadow-lg" />
            </div>
                  ...
                  return (
                    <div key={idx} className="bg-gradient-to-br from-white to-gray-50 rounded-2xl p-3 md:p-4 border-2 border-gray-200 hover:border-purple-400 hover:shadow-xl transition-all cursor-pointer group">
                      <div className="flex items-start justify-between mb-2 md:mb-3">
                        ...
                        <div className={`${getBadgeColor(media)} px-2 md:px-3 py-1 rounded-full text-white font-bold text-xs md:text-sm shadow-md`}>{media} ⭐</div>
                      </div>
                      {emp.comment && (
